...
const todosMocks = vi.hoisted(() => {
  ...
  const createTodoMutationOptions = vi.fn((queryClient: {
    invalidateQueries: (options: { exact: boolean; queryKey: typeof listQueryKey }) => Promise<void>;
  }) => ({
  ...
  const updateTodoMutationOptions = vi.fn((queryClient: {
    invalidateQueries: (options: { exact: boolean; queryKey: typeof listQueryKey }) => Promise<void>;
  }) => ({
  ...
  const removeTodoMutationOptions = vi.fn((queryClient: {
    invalidateQueries: (options: { exact: boolean; queryKey: typeof listQueryKey }) => Promise<void>;
  }) => ({
...
describe("TodosPage", () => {
  ...
  it("creates a todo, invalidates the list query, and resets the input on success", async () => {
    ...
    expect((screen.getByLabelText("Todo title") as HTMLInputElement).value).toBe("");
  });
  ...
  it("shows inline update or delete errors while keeping the last good list rendered", async () => {
    ...
    expect(await screen.findByText("Failed to update todo")).toHaveClass("error");
    expect(screen.getByText("Existing todo")).toBeInTheDocument();
    ...
    expect(await screen.findByText("Failed to delete todo")).toHaveClass("error");
    expect(screen.getByText("Existing todo")).toBeInTheDocument();
  });
  ...
  it("shows an inline create error and keeps the current todos visible on failure", async () => {
    ...
    expect(error).toHaveClass("error");
    expect(screen.getByText("Existing todo")).toBeInTheDocument();
    expect((screen.getByLabelText("Todo title") as HTMLInputElement).value).toBe("Broken todo");
  });
});
